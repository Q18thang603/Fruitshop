...
export default function Navbar({ search, setSearch }) {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'shadow-md' : ''}`}>
      ...
      {/* Main Navbar */}
      <div className={`bg-white/95 backdrop-blur-md py-4 border-b border-slate-100`}>
        <div className="container mx-auto px-4">
          {/* Main row */}
          <div className="flex items-center justify-between gap-8 mb-4">
            {/* Logo */}
            <Link to="/" className="flex flex-col">
              <span className="text-2xl font-black text-green-600 tracking-tighter leading-none flex items-center gap-1">
                GreenGo
                <span className="w-2 h-2 rounded-full bg-green-500 mb-2" />
              </span>
              <span className="text-[9px] font-bold text-slate-400 tracking-[0.2em] leading-none mt-1 uppercase">Trái cây & Thực phẩm hữu cơ</span>
            </Link>
            ...
            <div className="hidden md:flex flex-1 max-w-2xl relative items-center">
              <div className="flex w-full items-center">
                <Input
                  ...
                  className="rounded-l-full rounded-r-none border-slate-200 border-r-0 hover:border-green-400 focus:border-green-500 py-2.5 px-6 w-full"
                  value={search}
                ...
                <Button
                  ...
                  className="rounded-r-full rounded-l-none h-[46px] px-6 bg-green-600 hover:bg-green-700 border-none flex items-center justify-center"
                />
              </div>
                        ...
                        {filteredProducts.map((p) => (
                          ...
                          >
                            ...
                            <div>
                              ...
                              <p className="text-xs text-green-600 font-bold">{p.price.toLocaleString()} đ</p>
                            </div>
                          </Link>
                        ))}
                        <Link to="/products" className="block text-center py-2 text-xs text-slate-400 hover:text-green-600 transition-colors border-t border-slate-50 mt-1">
                          Xem tất cả sản phẩm
                        </Link>
                      </div>
                    ) : (
            ...
            <div className="flex items-center gap-2 md:gap-6">
              ...
              <div className="flex items-center gap-1 md:gap-4">
                ...
                <Link to="/cart">
                  <Badge count={totalQuantity} size="small" offset={[-4, 4]} color="#22c55e">
                    <div className="h-10 w-10 flex items-center justify-center rounded-full bg-green-50 hover:bg-green-100 transition-colors group">
          ...
          <div className="hidden md:flex items-center justify-between border-t border-slate-100 pt-4">
            <div className="relative">
              <button
                ...
                className="flex items-center gap-3 bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-t-2xl font-black transition-all"
              >
                    ...
                    >
                      {departments.map((dept, i) => (
                        <Link
                          key={i}
                          to={dept.path}
                          className="flex items-center justify-between px-6 py-3.5 text-slate-600 hover:bg-green-50 hover:text-green-600 transition-all font-medium border-b border-slate-50 last:border-none"
                        >
                          {dept.name}
            ...
            <nav className="flex items-center gap-8">
              {[
                ...
                { path: '/category/promo', label: 'Khuyến mãi' },
                ...
                return (
                  <Link
                    key={i}
                    to={link.path}
                    className={`text-sm font-black uppercase tracking-wider py-2 relative group ${isActive ? 'text-green-600 border-b-2 border-green-600' : 'text-slate-600 hover:text-green-600 transition-colors'}`}
                  >
                    {link.label}
                    {!isActive && <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-green-500 transition-all duration-300 group-hover:w-full" />}
                  </Link>
                );
              })}
            </nav>
 
            <Link to="/category/promo" className="flex items-center gap-2 text-green-700 font-extrabold text-sm bg-green-50 hover:bg-green-100 px-4 py-2 rounded-full transition-all">
          ...
          >
            ...
            <nav className="flex flex-col gap-6 overflow-y-auto">
              {['/', '/about', '/products', '/contact'].map((path, i) => (
                ...
                >
                  {path === '/' ? 'Trang chủ' : path.slice(1) === 'about' ? 'Giới thiệu' : path.slice(1) === 'products' ? 'Sản phẩm' : 'Liên hệ'}
                </Link>
              ))}
